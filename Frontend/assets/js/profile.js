...
(function () {
  ...
  const providerServicesBody = document.getElementById('provider-services-body');
  ...
  function renderBasicInfo(user) {
    const name = user.user_full_name || `${user.first_name || ''} ${user.last_name || ''}`.trim() || 'User';
    ...
    const roleBadgeClass = role === 'provider' ? 'badge-accent' : role === 'admin' ? 'badge-danger' : 'badge-info';
  ...
  async function loadProviderInfo(userData) {
    const data = await apiFetch(`/providers/${userData.id || userData.user_id}/`);
    if (!data || data.error) {
    ...
    const bio = data.bio || data.about || data.description || 'No bio provided.';
    ...
    if (skills.length) {
      ...
      providerSkillsBody.innerHTML = '<p class="text-muted">No skills added yet.</p>';
    }

    const services = data.services || [];
    if (services.length) {
      providerServicesBody.innerHTML = services.map(s => `
      ...
      providerServicesBody.innerHTML = '<p class="text-muted">No services added yet.</p>';
    }
  }
