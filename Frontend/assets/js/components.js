...
function renderTopbar() {
  ...
  if (!role) {
    ...
    navLinks = `
      ...
      <a href="/pages/admin-skills.html" class="nav-link ${isActive("admin-skills.html")}">Skills</a>
  ...
  topbar.innerHTML = `
    ...
    <div class="mobile-nav" id="mobile-nav">
      ${navLinks
        .split("\n")
        .filter((l) => l.trim())
        .map((l) => l.replace(/class="nav-link/g, 'class="nav-link'))
        .join("\n")}
...
function buildProviderCard(provider) {
  const name =
    provider.user_full_name ||
    `${provider.first_name || ""} ${provider.last_name || ""}`.trim() ||
    "Provider";
  const avatar =
    provider.user_avatar || provider.avatar || provider.profile_image || "";
  ...
  const title =
    provider.tagline || provider.business_name || provider.service_title || "";
        ...
        ${
          ...
          <div class="skill-list" style="margin-bottom:var(--sp-3)">
            ${skills
              ...
              .map(
                (s) =>
                  `<span class="skill-tag">${typeof s === "string" ? s : s.skill_name || s.name || ""}</span>`,
              )
              .join("")}
          </div>
        `
...
function buildBookingCard(booking, role) {
  const service =
    booking.service_name || booking.service?.name || booking.title || "Service";
